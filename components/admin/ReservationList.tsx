...
interface AdminReservationRow {
  ...
  reservation_addons: ReservationAddonRow[];
}

function describeAddons(items: ReservationAddonRow[]): string {
  return items
    .map((item) => `${item.addons?.name ?? "옵션"} × ${item.quantity}`)
    .join(", ");
}
...
export default function ReservationList() {
  ...
  const fetchReservations = useCallback(
    async (target: FilterState, kw: string, targetPage: number) => {
      setLoading(true);
      try {
        const { reservations, total: totalCount } = await loadReservationRows(
          target,
          kw,
          targetPage,
        );
  ...
  const performCancel = useCallback(async (id: string, reason: string) => {
    ...
    try {
      ...
      setCancelReasonError(
        err instanceof Error ? err.message : "취소 처리에 실패했습니다.",
      );
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <div>
          <label htmlFor="created_from" className="form-label">
            접수일(부터)
          </label>
          <input
            type="date"
        ...
        <div>
          <label htmlFor="created_to" className="form-label">
            접수일(까지)
          </label>
          <input
            type="date"
      ...
      {loading ? (
        ...
        <p className="card p-6 text-sm text-center text-muted">
          조건에 맞는 예약이 없습니다.
        </p>
      ) : (
        <>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      {r.reservation_addons.length > 0 ? (
                        <button
                          type="button"
                          onClick={() =>
                            setVaild(
                              `선택한 옵션: ${describeAddons(r.reservation_addons)}`,
                            )
                          }
                          className="badge badge-info cursor-pointer"
                        >
                          {r.reservation_addons.length}개
                        </button>
                      ) : (
                    ...
                    <td>
                      ...
                      {r.status === "CANCELLED" && r.cancel_reason && (
                        <button
                          type="button"
                          onClick={() =>
                            setVaild(`취소 사유: ${r.cancel_reason}`)
                          }
          ...
          >
            <div className="px-6 pt-6 pb-5">
              ...
              <p className="text-sm text-muted pb-4 mb-4 border-b border-gray-100">
                환불 규정에 따라 환불 금액이 자동 계산됩니다. 취소 사유를
                입력해주세요.
              </p>
